...
#write functionality to get our trade decisions

#candle_time below is the most recent candle time detected
ADDROWS = 20
...
def process_candles(df:pd.DataFrame, pair, trade_settings:TradeSettings, log_message):  #used to identify trade signals
    ...
    log_cols = ['PAIR','time','mid_c','mid_o','SL','TP','SPREAD','GAIN','LOSS','SIGNAL'] #columns we want to log
...
def get_trade_decision(candle_time, pair, granularity, api: OandaApi, trade_settings: TradeSettings, log_message):
    max_rows = trade_settings.n_ma + ADDROWS  #number of candle request from the api
